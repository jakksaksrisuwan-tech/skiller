...
@dataclass(frozen=True)
class TypingSnippet:
    """Single-line snippet for the structure-driven typing drill.

    `language` separates pools (e.g. 'python' vs 'linux').
    `structure` is the construct/idiom drilled (e.g. 'lambda', 'find_filter').
    ...
    language: str = "python"
    difficulty: int = 2
    description: str = ""  # short human explanation shown above the snippet box
    tags: list[str] = field(default_factory=list)

    @property
    def is_synthetic_drill(self) -> bool:
        """True for snippets generated by F4 correction mode (e.g. 'drill[st]').
        These should not be recorded against per-structure stats."""
        return self.structure.startswith("drill[")
